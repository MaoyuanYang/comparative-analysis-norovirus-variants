...
for i in range(SeqLen):
    ...
    gapNum = 0
    num = 0
ORF1 = {'synonymous': 0, 'NonSynonymous': 0}
ORF2 = {'synonymous': 0, 'NonSynonymous': 0}
ORF3 = {'synonymous': 0, 'NonSynonymous': 0}
data = {'synonymous': [], 'NonSynonymous': []}
synonymous = 0
NonSynonymous = 0
if CapsidSubtype == 'Sydney':
    for site in x:
        if 5 <= site <= 5110:
            ...
            if flag == 2:
                for seq in seqs:
                    codon = seq[site] + seq[site + 1] + seq[site + 2]
                    comparedCodon = ComparedSeq[site] + ComparedSeq[site + 1] + ComparedSeq[site + 2]
                    if Seq(codon).translate() == Seq(comparedCodon).translate():
                        synonymous = synonymous + 1
                    else:
                        NonSynonymous = NonSynonymous + 1
                    data['synonymous'].append(synonymous)
                    data['NonSynonymous'].append(NonSynonymous)
                    synonymous = 0
                    NonSynonymous = 0
            if flag == 0:
